...
async function logoutUser(req, res, next) {
  ...
  try {
    ...
    if (!!bcrypt.compareSync(password, user.password)) {
      ...
      return res.status(201).json(`User ${req.body.email} has logged out, token: ${req.headers.authorization}`)
    } else {
      return next(new Error("Incorrect password!"));
    }
  } catch (err) {
...
async function changePasswordUser(req, res, next) {
  ...
  try {
    ...
    if (!!!user) return next(new Error("User not found"));
    if (!!bcrypt.compareSync(password, user.password)) {
      if (!!bcrypt.compareSync(newPassword, user.password)) return next(new Error("Can't change with same password"))
